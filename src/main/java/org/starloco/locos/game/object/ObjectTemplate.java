package org.starloco.locos.game.object;
...
public class ObjectTemplate {
    ...
    public GameObject createNewCertificat(GameObject obj) {
        ...
        if (getType() == Constant.ITEM_TYPE_CERTIFICAT_CHANIL) {
            PetEntry myPets = World.world.getPetsEntry(obj.getId());
            ...
            World.world.removePetsEntry(obj.getId());
            Database.getStatics().getPetData().delete(obj.getId());
        }
        return item;
    }
    ...
    public GameObject createNewItemWithoutDuplication(Collection<GameObject> objects, int qua) {
        int id = -1;
        GameObject item;
        if (getType() == Constant.ITEM_TYPE_QUETES && (Constant.isCertificatDopeuls(getId()) || getId() == 6653)) {
            ...
            item = new GameObject(id, getId(), 1, Constant.ITEM_POS_NO_EQUIPED, (new Stats(false, null)), new ArrayList<>(), new HashMap<>(), World.world.getPets(getId()).generateNewtxtStatsForPets(), 0);
            //Ajouter du Pets_data SQL et World
            long time = System.currentTimeMillis();
            World.world.addPetsEntry(new PetEntry(id, getId(), time, 0, 10, 0, false));
            Database.getStatics().getPetData().add(id, time, getId());
        } else if(getType() == Constant.ITEM_TYPE_CERTIF_MONTURE) {
            item = new GameObject(id, getId(), qua, Constant.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(getStrTemplate(), false), getEffectTemplate(getStrTemplate()), new HashMap<>(), new HashMap<>(), 0);
        } else {
            if (getType() == Constant.ITEM_TYPE_OBJET_ELEVAGE) {
                item = new GameObject(id, getId(), qua, Constant.ITEM_POS_NO_EQUIPED, new Stats(false, null), new ArrayList<>(), new HashMap<>(), getStringResistance(getStrTemplate()), 0);
            } else if (Constant.isIncarnationWeapon(getId())) {
                ...
                Stats.put(Constant.ERR_STATS_XP, 0);
                Stats.put(Constant.STATS_NIVEAU, 1);
                item = new GameObject(id, getId(), qua, Constant.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(getStrTemplate(), false), getEffectTemplate(getStrTemplate()), Stats, new HashMap<>(), 0);
            } else {
                Map<Integer, String> Stat = new HashMap<>();
                switch (getType()) {
                    case 1:
                ...
                item = new GameObject(id, getId(), qua, Constant.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(getStrTemplate(), false), getEffectTemplate(getStrTemplate()), new HashMap<Integer, Integer>(), Stat, 0);
                item.getSpellStats().addAll(this.getSpellStatsTemplate());
            }
        }
