...
public class GameObject {
    ...
    public GameObject(int id, int template, int qua, int pos, String strStats, int puit) {
        this.id = id;
    ...
    public GameObject(int id, int template, int qua, int pos, Stats stats, ArrayList<SpellEffect> effects, Map<Integer, Integer> _SoulStat, Map<Integer, String> _txtStats, int puit) {
        this.id = id;
        this.template = World.world.getObjTemplate(template);
        this.quantity = qua;
        this.position = pos;
    ...
    public int setId() {
        this.id = Database.getStatics().getObjectData().getNextId();
        return this.getId();
    }
    ...
    public void setQuantity(int quantity) {
        if (quantity <= 0)
            ...
            Loggers.objects.warn("Faille : Objet id : " + id + " a dépassé 100 000 qua (" + quantity + ") avec comme template : " + template.getName() + " (" + template.getId() + ")");
    ...
    public String parseStatsString() {
        ...
        for (Entry<Integer, String> entry : txtStats.entrySet()) {
            ...
            if (template.getType() == 77 || template.getType() == 90) {
                ...
                PetEntry p = World.world.getPetsEntry(this.getId());
                if (p == null) {
                    if (entry.getKey() == Constant.STATS_PETS_PDV)
    ...
    public void removeAllObvijevanStats() {
        setObvijevanPos(0);
        org.starloco.locos.game.client.other.Stats StatsSansObvi = new Stats();
        for (Entry<Integer, Integer> entry : Stats.getEffects().entrySet()) {
