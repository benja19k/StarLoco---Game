package org.starloco.locos.game.job;
...
public class JobAction {
    ...
    public void endAction(Player player, InteractiveObject IO, GameAction GA, GameCase cell) {
        ...
        if (!this.isCraft) {
            ...
            if (SM.getTemplate().getId() == 36) {
                if (qua > 0)
                    SM.addXp(player, (long) (this.getXpWin() * Config.INSTANCE.getRateJob() * World.world.getConquestBonus(player)));
            } else
                SM.addXp(player, (long) (this.getXpWin() * Config.INSTANCE.getRateJob() * World.world.getConquestBonus(player)));
    ...
    public boolean craftPublicMode(Player crafter, Player receiver, Map<Player, ArrayList<Couple<Integer, Integer>>> list) {
        ...
        Loggers.craft.debug(this.player.getName() + " à crafter avec " + (success ? "SUCCES" : "ECHEC") + " l'item " + template + " (" + World.world.getObjTemplate(template).getName() + ") pour " + receiver.getName());
        if (!success) {
        ...
        int winXP = Formulas.calculXpWinCraft(SM.get_lvl(), this.ingredients.size()) * Config.INSTANCE.getRateJob();
    ...
    synchronized void craft(boolean isRepeat) {
        ...
        if (!isUnjobSkill) {
            ...
            Loggers.craft.info(this.player.getName() + " à crafter avec " + (success ? "SUCCES" : "ECHEC") + " l'item " + templateId + " (" + World.world.getObjTemplate(templateId).getName() + ")");
            if (!success) {
                SocketManager.GAME_SEND_Ec_PACKET(this.player, "EF");
                SocketManager.GAME_SEND_IO_PACKET_TO_MAP(this.player.getCurMap(), this.player.getId(), "-" + templateId);
                SocketManager.GAME_SEND_Im_PACKET(this.player, "0118");
            } else {
                GameObject newObj = World.world.getObjTemplate(templateId).createNewItemWithoutDuplication(this.player.getItems().values(), 1);
            ...
            int winXP = 0;
            if (success)
                winXP = Formulas.calculXpWinCraft(SM.get_lvl(), this.ingredients.size()) * Config.INSTANCE.getRateJob();
    ...
    /* ********FM TOUT POURRI*************/
     private synchronized boolean craftMaging(boolean isRepeat, Player receiver, Map<Player, ArrayList<Couple<Integer, Integer>>> items) {
            ...
            switch (template) {
                ...
                default:
                    ...
                    if ((type >= 1 && type <= 11) || (type >= 16 && type <= 22) || type == 81 || type == 102 || type == 114 || object.getTemplate().getPACost() > 0) {
                        final Player player = this.player.hasItemGuid(object.getId()) ? this.player : receiver;
                        objectFm = object;
                        SocketManager.GAME_SEND_EXCHANGE_OTHER_MOVE_OK_FM(player.getGameClient(), 'O', "+", objectFm.getId() + "|" + 1);
                        deleteID = id;
        ...
        if (successC || successN) {
            int winXP = Formulas.calculXpWinFm(objectFm.getTemplate().getLevel(), poid) * Config.INSTANCE.getRateJob();
            if (winXP > 0) {
                SM.addXp(this.player, winXP);
                ArrayList<JobStat> SMs = new ArrayList<>();
                SMs.add(SM);
        ...
        if(receiver == null) {
            ...
            if(items != null) {
                for(Entry<Player, ArrayList<Couple<Integer, Integer>>> entry : items.entrySet()) {
                    ...
                    for(Couple<Integer, Integer> couple : entry.getValue()) {
                        if(signingRune != null && signingRune.getId() == couple.first)
                            this.decrementObjectQuantity(player, signingRune);
                        if(runeOrPotion.getId() == couple.first)
    ...
    private void decrementObjectQuantity(Player player, GameObject object) {
        if (object != null) {
            ...
            if (newQua <= 0) {
                ...
                SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(player, object.getId());
            } else {
    ...
    /*private synchronized void craftMaging(boolean isReapeat, int repeat) {
        ...
        /*if(runeObject != null) {
            ...
            if (result == 0) {
                ...
                int winXP = Formulas.calculXpWinFm(gameObject.getTemplate().getLevel(), (int) Math.floor(runeTemplate.getWeight())) * Config.INSTANCE.getRateJob;
