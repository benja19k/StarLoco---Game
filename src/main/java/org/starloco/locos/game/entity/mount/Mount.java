package org.starloco.locos.game.entity.mount;
...
public class Mount {
	...
	public void addObject(int guid, int qua, Player P) {
		...
		if(TrunkObj == null)//S'il n'y pas d'item du meme Template
		{
			...
			if(newQua <= 0)
			{
				...
				str = "O+"+playerObj.getId()+"|"+playerObj.getQuantity()+"|"+playerObj.getTemplate().getId()+"|"+playerObj.parseStatsString();
		...
		}else // S'il y avait un item du meme template
		{
			...
			if(newQua <= 0)
			{
				...
				World.world.removeGameObject(playerObj.getId());
	...
	public void removeObject(int guid, int qua, Player P) {
		...
		}else // Le joueur avait deja� un item similaire
		{
			...
			if(newQua <= 0)
			{
				...
				World.world.removeGameObject(TrunkObj.getId());
	...
	public String parseObjectsToString() {
		String str = "";
		for(GameObject gameObject : this.objects.values())
			str += (str.isEmpty() ? "" : ";") + gameObject.getId();
		return str;
	}
