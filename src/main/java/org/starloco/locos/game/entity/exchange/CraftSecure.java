package org.starloco.locos.game.entity.exchange;
...
public class CraftSecure extends PlayerExchange {
    ...
    public synchronized void apply() {
        ...
        int winXP = 0;
        if (success)
            winXP = Formulas.calculXpWinCraft(jobStat.get_lvl(), sizeList) * Config.INSTANCE.getRateJob();
    ...
    private void addItem(GameObject object, int quantity, byte type) {
        ...
        Couple<Integer, Integer> couple = getCoupleInList(items, object.getId());
        String add = "|" + object.getTemplate().getId() + "|" + object.parseStatsString();

        if (couple != null) {
            couple.second += quantity;
            this.player2.send("Ep" + type + ";O+" + object.getId() + "|" + couple.second);
            this.player1.send("Ep" + type + ";O+" + object.getId() + "|" + couple.second + add);
            return;
        }

        items.add(new Couple<>(object.getId(), quantity));
    ...
    private void removeItem(GameObject object, int quantity, byte type) {
        ...
        if (newQua < 1) {
            ...
            this.player1.send("Ep" + type + ";O+" + object.getId() + "|" + newQua + "|" + object.getTemplate().getId() + "|" + object.parseStatsString());
        }
    }
