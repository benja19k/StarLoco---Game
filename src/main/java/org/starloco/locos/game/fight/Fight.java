package org.starloco.locos.game.fight;

import org.starloco.locos.game.area.SubArea;
...
import org.starloco.locos.game.dynamic.FormuleOfficiel;
...
import org.starloco.locos.game.entity.monster.boss.Bandit;
...
import org.starloco.locos.game.fight.arena.FightManager;
import org.starloco.locos.game.fight.arena.TeamMatch;
import org.starloco.locos.game.fight.ia.IAHandler;
import org.starloco.locos.game.fight.spells.LaunchedSpell;
import org.starloco.locos.game.fight.spells.Spell.SortStats;
import org.starloco.locos.game.fight.spells.SpellEffect;
import org.starloco.locos.game.fight.traps.Glyph;
import org.starloco.locos.game.fight.traps.Trap;
import org.starloco.locos.game.fight.turn.Turn;
...
import org.starloco.locos.game.world.world.World.Drop;
                                    ...
                                    if (caster.getPlayer().getId() != fighter.getPlayer().getId()) {// Celui qui a join le fight revient sur la map
                                        ...
                                        if (this.getType() == Constant.FIGHT_TYPE_AGRESSION || getType() == Constant.FIGHT_TYPE_PVM || getType() == Constant.FIGHT_TYPE_PVT || getType() == Constant.FIGHT_TYPE_CONQUETE) {
                                            ...
                                            if (caster.getPlayer().getObjetByPos(Constant.ITEM_POS_FAMILIER) != null) {
                                                ...
                                                if (obj != null) {
                                                    PetEntry pets = World.world.getPetsEntry(obj.getId());
                                                    if (pets != null)
    ...
    void onPlayerWin(Fighter fighter, ArrayList<Fighter> looseTeam) {
        ...
        if (this.getType() == Constant.FIGHT_TYPE_PVM) {
            ...
            if (obj != null) {
                ...
                if (!souls.isEmpty()) {
                    PetEntry pet = World.world.getPetsEntry(obj.getId());
                    if (pet != null)
                        ...
                        try {
                            ...
                            if(f != null && f.getPlayer() != null) {
                                ...
                                if (Formulas.getRandomValue(1, 100) <= Formulas.totalCaptChance(playerSoulStone.first, f.getPlayer())) {// Si le joueur obtiens la capture Retire la pierre vide au personnage et lui envoie ce changement
                                    int emptySoulStone = f.getPlayer().getObjetByPos(Constant.ITEM_POS_ARME).getId();
                    ...
                    for (int i = 0; i < getTrainer().size(); i++) {
                        try {
                            ...
                            if (chance <= appriChance) {
                                ...
                                int filet = player.getObjetByPos(Constant.ITEM_POS_ARME).getId();
                    ...
                    if (player != null || (i.getMob() != null && i.getMob().getTemplate().getId() == 285)) {
                        ...
                        for (Entry<Integer, Integer> entry : objectsWon.entrySet()) {
                            ...
                            if (objectTemplate.getType() == 32 && player != null) {
                                ...
                                GameObject newObj = World.world.getObjTemplate(objectTemplate.getId()).createNewItemWithoutDuplication(target.getItems().values(), entry.getValue());
            ...
            if (Collector.getCollectorByMapId(getMap().getId()) != null && getType() == Constant.FIGHT_TYPE_PVM) {
                ...
                if (collector.getPodsTotal() < collector.getMaxPod()) {
                    ...
                    for (Entry<Integer, Integer> entry : objectsWon.entrySet()) {
                        ...
                        GameObject newObj = World.world.getObjTemplate(objectTemplate.getId()).createNewItemWithoutDuplication(collector.getOjects().values(), entry.getValue());
                        int guid = newObj.getId();//FIXME: Ne pas recrée un item pour l'empiler après
