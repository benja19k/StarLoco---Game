...
import org.starloco.locos.game.area.map.labyrinth.Minotoror;
import org.starloco.locos.game.area.map.labyrinth.PigDragon;
import org.starloco.locos.game.client.other.Party;
import org.starloco.locos.game.client.other.Restriction;
import org.starloco.locos.game.client.other.Stalk;
import org.starloco.locos.game.client.other.Stats;
...
import org.starloco.locos.game.dynamic.Start;
...
import org.starloco.locos.game.fight.arena.DeathMatch;
import org.starloco.locos.game.fight.arena.TeamMatch;
import org.starloco.locos.game.fight.spells.Spell;
import org.starloco.locos.game.fight.spells.SpellEffect;
...
import org.starloco.locos.core.main.Reboot;
...
public class Player {
    ...
    public void setRoleplayBuff(int id) {
        ...
        if (getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF) != null) {
            int guid = getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getId();
    ...
    public void setBenediction(int id) {
        if (getObjetByPos(Constant.ITEM_POS_BENEDICTION) != null) {
            int guid = getObjetByPos(Constant.ITEM_POS_BENEDICTION).getId();
            SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this, guid);
            this.deleteItem(guid);
        }
        if (id == 0) {
    ...
    public void setMalediction(int id) {
        ...
        if (getObjetByPos(Constant.ITEM_POS_MALEDICTION) != null) {
            int guid = getObjetByPos(Constant.ITEM_POS_MALEDICTION).getId();
            SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this, guid);
            this.deleteItem(guid);
        }
    ...
    public void setMascotte(int id) {
        if (getObjetByPos(Constant.ITEM_POS_PNJ_SUIVEUR) != null) {
            int guid = getObjetByPos(Constant.ITEM_POS_PNJ_SUIVEUR).getId();
            SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this, guid);
            this.deleteItem(guid);
        }
        if (id == 0) {
    ...
    public void setCandy(int id) {
        if (getObjetByPos(Constant.ITEM_POS_BONBON) != null) {
            int guid = getObjetByPos(Constant.ITEM_POS_BONBON).getId();
            SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this, guid);
            this.deleteItem(guid);
        }
        int turn = 30;
    ...
    public void OnJoinGame() {
        ...
        for (GameObject object : this.objects.values()) {
            if (object.getTemplate().getType() == Constant.ITEM_TYPE_FAMILIER) {
                PetEntry p = World.world.getPetsEntry(object.getId());
    ...
    public void removeItem(int guid, int nombre, boolean send,
        ...
        if (obj.getQuantity() >= nombre) {
            ...
            if (newQua > 0) {
                ...
                if (deleteFromWorld)
                    World.world.removeGameObject(obj.getId());
    ...
    public GameObject getObjetByPos(int pos) {
        ...
        for (GameObject gameObject : this.objects.values()) {
            if (gameObject.getPosition() == pos && pos == Constant.ITEM_POS_FAMILIER) {
                ...
                else if (World.world.getPetsEntry(gameObject.getId()) == null) return null;
            }
    ...
    public void addInBank(int guid, int qua) {
        ...
        // S'il y avait un item du meme template
        {
            ...
            {
                removeItem(PersoObj.getId()); //On enleve l'objet du sac du joueur
                World.world.removeGameObject(PersoObj.getId()); //On enleve l'objet du monde
    ...
    public void removeFromBank(int guid, int qua) {
        ...
        if (PersoObj == null)//Si le joueur n'avait aucun item similaire
        {
            ...
            if (newQua <= 0) {
                //On retire l'item de la banque
                account.getBank().remove(index);
                World.world.removeGameObject(BankObj.getId());
            ...
            //Si pas assez d'objet
            {
                if (obj.getQuantity() >= tempCount) {
                    ...
                    for (GameObject o : remove) {
                        ...
                        World.world.removeGameObject(o.getId());
                        //on envoie le packet si connect�
                        if (isOnline)
    ...
    public void die(byte type, long id) {
        new ArrayList<>(this.getItems().values()).stream().filter(object -> object != null).forEach(object -> this.removeItem(object.getId(), object.getQuantity(), true, false));
        this.setFuneral();
        this.deathCount++;
        this.deadType = type;
    ...
    public void revive() {
        ...
        if(this._mount != null) {
            for(GameObject gameObject : this._mount.getObjects().values())
                World.world.removeGameObject(gameObject.getId());
    ...
    public void addInStore(int ObjID, int price, int qua) {
        ...
        if (SimilarObj == null)//S'il n'y pas d'item du meme Template
        {
            ...
            if (newQua <= 0) {
                ...
                SocketManager.GAME_SEND_REMOVE_ITEM_PACKET(this, PersoObj.getId());
                SocketManager.GAME_SEND_ITEM_LIST_PACKET_SELLER(this, this);
            } else
            //S'il reste des objets au joueur
            {
        ...
        // S'il y avait un item du meme template
        {
            ...
            if (newQua <= 0) {
                ...
                World.world.removeGameObject(PersoObj.getId());
    ...
    public void removeFromStore(int guid, int qua) {
        ...
        if (PersoObj == null)//Si le joueur n'avait aucun item similaire
        {
            ...
            if (newQua <= 0) {
                //On retire l'item de la banque
                _storeItems.remove(SimilarObj.getId());
                World.world.removeGameObject(SimilarObj.getId());
                //On Modifie la quantit� de l'item du sac du joueur
                PersoObj.setQuantity(PersoObj.getQuantity()
    ...
    public boolean addObjetSimiler(GameObject objet, boolean hasSimiler, int oldID) {
        ...
        if (hasSimiler) {
            for (Entry<Integer, GameObject> entry : objects.entrySet()) {
                GameObject obj = entry.getValue();
                if (obj.getPosition() == -1 && obj.getId() != oldID
